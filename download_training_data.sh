...
if [ ! -e "images_upright" ]; then
    ...
    fname=/content/drive/MyDrive/database_and_query_images.zip
    if [ ! -f $fname ]; then
...
if [ ! -e "style_transfer" ]; then
    ...
    if [ ! -f $fname ]; then
        wget http://download.europe.naverlabs.com/3DVision/aachen_style_transfer.zip -O $fname
    fi
    unzip $fname
    rm -f $fname
fi
...
if [ ! -e "optical_flow" ]; then
    ...
    if [ ! -f $fname ]; then
        wget http://download.europe.naverlabs.com/3DVision/aachen_optical_flow.zip -O $fname
    fi
    unzip $fname
    rm -f $fname
fi
